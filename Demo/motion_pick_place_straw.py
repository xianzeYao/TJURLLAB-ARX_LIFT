...
def make_open_action(pt_ref: Optional[np.ndarray], arm: str) -> Dict[str, np.ndarray]:
    ...
    base = np.zeros(3, dtype=np.float32) if pt_ref is None else pt_ref
    active = np.array(
        [base[0]-GRIPPER_OFFSET-0.02, base[1], base[2]+Z_STRAW-0.1, 0, 0, 0, OPEN],
        dtype=np.float32,
    )
    return _make_arm_action(arm, active)
...
def build_pick_straw_sequence(pt_ref: Optional[np.ndarray], arm: str):
    """返回抓取动作序列，不执行。"""
    return [
...
def build_place_straw_sequence(pt_ref: Optional[np.ndarray], arm: str):
    """返回放置动作序列，不执行。"""
    return [
